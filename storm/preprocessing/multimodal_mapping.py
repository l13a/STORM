...
def get_gene_annotations(
    ...
    try:
        from storm import genomics
    except ImportError:
        raise ImportError("optional dependency `storm` is required for this function")
    ...
    if adata_atac is not None:
        split = adata_atac.var_names.str.split(r"[_]")
...
def add_multimodal_mask_to_adata(
        ...
        gene_peaks_mask_key: str="storm_gene_peaks",
        gp_targets_mask_key: str="storm_gp_targets",
        gp_sources_mask_key: str="storm_gp_sources",
        gp_names_key: str="storm_gp_names",
        ca_targets_mask_key: str="storm_ca_targets",
        ca_sources_mask_key: str="storm_ca_sources",
        source_peaks_idx_key: str="storm_source_peaks_idx",
        target_peaks_idx_key: str="storm_target_peaks_idx",
        peaks_idx_key: str="storm_peaks_idx") -> Tuple[AnnData, AnnData]:
    """
    ...
    # highly_variable_peaks = adata_atac.var_names[adata_atac.var["highly_variable"]]
    # adata_atac = adata_atac[:, highly_variable_peaks]

    # # Filter peaks in gene_peak_mapping_dict to include only peaks in filtered adata_atac.var_names
    # valid_peaks = set(adata_atac.var_names)
    # filtered_gene_peak_mapping_dict = {
    #     gene: [peak for peak in peaks if peak in valid_peaks]
    #     for gene, peaks in gene_peak_mapping_dict.items()
    # }

    if filter_peaks_based_on_genes:
        all_gene_peaks = list(
            # set(peak for gene_peaks in filtered_gene_peak_mapping_dict.values() for peak
    ...
    gene_idx_peak_idx_mapping_dict = {
        ...
        # for gene, peaks in filtered_gene_peak_mapping_dict.items()}
        for gene, peaks in gene_peak_mapping_dict.items()}
    ...
    for entity in ["targets", "sources"]:
        ...
        if entity == "targets":
            ...
            all_target_gp_peaks = [list(set([peak for gene_peaks in
                                    # [filtered_gene_peak_mapping_dict.get(gene, []) for
