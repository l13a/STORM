...
from ._helpers import load_R_file_as_df, create_gp_gene_count_distribution_plots
...
def add_gps_from_gp_dict_to_adata(
        ...
        gp_targets_mask_key: str="storm_gp_targets",
        gp_targets_categories_mask_key: str="storm_gp_targets_categories",
        targets_categories_label_encoder_key: str="storm_targets_categories_label_encoder",
        gp_sources_mask_key: str="storm_gp_sources",
        gp_sources_categories_mask_key: str="storm_gp_sources_categories",
        sources_categories_label_encoder_key: str="storm_sources_categories_label_encoder",
        gp_names_key: str="storm_gp_names",
        source_genes_idx_key: str="storm_source_genes_idx",
        target_genes_idx_key: str="storm_target_genes_idx",
        genes_idx_key: str="storm_genes_idx",
    ...
    if filter_genes_not_in_masks:
        ...
        combined_gp_mask = np.maximum(adata.varm["storm_gp_sources"],
                                      adata.varm["storm_gp_targets"])
...
def extract_gp_dict_from_nichenet_lrt_interactions(
    ...
    for ligand in ligand_target_mask_dict.keys():
        ...
        # Merge ligand (sources) into targets without overriding
        gp_dict[ligand + "_ligand_receptor_target_gene_GP"]["targets"].extend([ligand])  # CHANGED: Extend targets with ligands
        
        gp_dict[ligand + "_ligand_receptor_target_gene_GP"]["sources"] = []
        ...
        gp_dict[ligand + "_ligand_receptor_target_gene_GP"][
            ...
            len(receptors)))
        gp_dict[ligand + "_ligand_receptor_target_gene_GP"]["targets_categories"].extend(["ligand"])  # CHANGED: Extend targets_categories with sources_categories
        gp_dict[ligand + "_ligand_receptor_target_gene_GP"]["sources_categories"] = []
...
def extract_gp_dict_from_omnipath_lr_interactions(
    ...
    # Drop rows where either side is missing so the dict below never contains
    # NaN/None values (which would break the "COMPLEX:" string check).
    lr_interaction_df = lr_interaction_df.dropna(
        subset=["genesymbol_intercell_source", "genesymbol_intercell_target"])
    ...
    lr_interaction_dict = {key: ([value] if "COMPLEX:" not in value
                             else value[len("COMPLEX:"):].split("_"))
    ...
    for ligand, receptors in lr_interaction_dict.items():
        gp_dict[ligand + "_ligand_receptor_GP"] = {
            ...
            "sources_categories": [],
            # "targets_categories": ["receptor"] * len(receptors)
            "targets": [ligand] + receptors,  # CHANGED: Combine ligand and receptors into "targets"
            "targets_categories": ["ligand"] + ["receptor"] * len(receptors)  # CHANGED: Combine categories
        }
    ...
    if species == "mouse":
        ...
        # or capitalize them if no orthologs are found
        for gp_name, gp in gp_dict.items():
            gp["targets_categories"] = [element for sublist in [[source_category] *
                                        len(grouped_mapping_df[
                                            grouped_mapping_df["Gene name"] == source][
                                            "Mouse gene name"].values[0])
                                        if source in grouped_mapping_df["Gene name"].values
                                        else [source_category]
                                        for source, source_category in zip(gp["targets"], gp["targets_categories"])] for element in sublist]
            gp["targets"] = [element for sublist in [grouped_mapping_df[grouped_mapping_df[
                "Gene name"] == source]["Mouse gene name"].values[0]
                             if source in grouped_mapping_df["Gene name"].values
                             else [source.capitalize()] for source in gp["targets"]] for element in sublist]
