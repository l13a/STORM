...
from typing import Any, List, Mapping, NoReturn, Optional, Tuple, Literal
...
from .dataset import ArrayDataset, DataLoader, GraphDataset, ParallelDataLoader
...
class GLUE(torch.nn.Module):
    ...
    du
        Modality and Timepoint discriminator
    ...
    def __init__(
            ...
            u2x_targets: Mapping[str, torch.nn.Module],
            u2x_sources: Mapping[str, torch.nn.Module],
            graph_decoder: torch.nn.Module,
            idx: Mapping[str, torch.Tensor],
            du: Discriminator, prior: Prior,
            node_label_aggregator: Mapping[str, torch.nn.Module],
            n_input: int,
        ...
        super().__init__()
        if not set(x2u.keys()) == set(u2x_targets.keys()) == set(u2x_sources.keys()) == set(idx.keys()) != set():
            raise ValueError(
        ...
        self.u2x_targets = torch.nn.ModuleDict(u2x_targets)
        self.u2x_sources = torch.nn.ModuleDict(u2x_sources)
        ...
        # self.du_t = du_t
        self.prior = prior
        self.node_label_aggregator = node_label_aggregator
        ...
        self.target_rna_decoder_addon_mask=target_rna_decoder_addon_mask,
        self.source_rna_decoder_addon_mask=source_rna_decoder_addon_mask,
        self.target_atac_decoder_addon_mask=target_atac_decoder_addon_mask,
        self.source_atac_decoder_addon_mask=source_atac_decoder_addon_mask,
        ...
        ############################################ VGPGAE #############################################
        # Check validity of mask indices (no overlaps)
        target_rna_idx_intersect = set(
            self.features_idx_dict_["target_masked_rna_idx"]).intersection(
            set(self.features_idx_dict_["target_unmasked_rna_idx"]))
        assert len(target_rna_idx_intersect) == 0
        source_rna_idx_intersect = set(
            self.features_idx_dict_["source_masked_rna_idx"]).intersection(
            set(self.features_idx_dict_["source_unmasked_rna_idx"]))
        assert len(source_rna_idx_intersect) == 0 
        
        if "atac" in idx:
            # Check validity of mask indices
            target_atac_idx_intersect = set(
                self.features_idx_dict_["target_masked_atac_idx"]).intersection(
                set(self.features_idx_dict_["target_unmasked_atac_idx"]))
            assert len(target_atac_idx_intersect) == 0
            source_atac_idx_intersect = set(
                self.features_idx_dict_["source_masked_atac_idx"]).intersection(
                set(self.features_idx_dict_["source_unmasked_atac_idx"]))
        ...
        self.register_buffer("running_mean_abs_mu",
                             torch.zeros(self.n_prior_gp_  + n_addon_gp))
        if "atac" in idx:
            self.register_buffer("running_mean_abs_mu_atac",
                                torch.zeros(self.n_prior_gp_  + n_addon_gp))

        # Initialize rna dynamic decoder masks
        self.target_rna_dynamic_decoder_mask = torch.ones(
            (self.n_prior_gp_ + n_addon_gp), self.n_output_genes_, dtype=torch.bool)
        ...
        if "atac" in idx:
            ...
            self.target_atac_dynamic_decoder_mask = torch.ones(
                (self.n_prior_gp_ + n_addon_gp), self.n_output_peaks_, dtype=torch.bool)
    ...
    **kwargs
    ...
    def val_step(
        ...
        if epoch <= self.n_epochs_rna:
            ...
            atac_only = False
            dsc_only = False
        elif self.n_epochs_rna < epoch <= self.n_epochs_rna + self.n_epochs_atac:
            rna_only = False
            atac_only = True
            dsc_only = False
        else:
        ...
        return self.compute_losses(data, epoch, dsc_only=dsc_only, rna_only=rna_only, atac_only=atac_only, use_only_active_gps=self.use_only_active_gps)
    ...
    def fit(  # pylint: disable=arguments-renamed
        ...
        if reduce_lr_patience:
            default_plugins.append(LRScheduler(
                ...
                burnin=self.align_burnin if safe_burnin else 0, n_epochs_rna=self.n_epochs_rna, n_epochs_atac=self.n_epochs_atac
            ))
        if patience:
            default_plugins.append(EarlyStopping(
                ...
                wait_n_lrs=wait_n_lrs or 0, n_epochs_rna=self.n_epochs_rna, n_epochs_atac=self.n_epochs_atac
            ))
