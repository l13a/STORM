...
from .._internal import config, get_rs, logged, processes
...
class AnnDataset(Dataset):
    ...
    *adatas
    ...
    def __getitem__(self, index: int) -> Tuple[List[torch.Tensor],np.ndarray]:
        s = slice(
            index * self.getitem_size,
            min((index + 1) * self.getitem_size, self.size)
        )
        ...
        items = []
        for i, extracted_data in enumerate(self.extracted_data):
            if i == len(self.extracted_data) - 1:
                adj_matrices = extracted_data  # This is a list of 2 adjacency matrices
                for j, adj_matrix in enumerate(adj_matrices):
                    # Use the corresponding shuffle_idx for each adjacency matrix (j = 0 or 1)
                    current_shuffle_idx = shuffle_idx[j].flatten()  # Flatten to 1D integer indices
                    
                    # Subset the adjacency matrix directly using the integer indices
                    adj_subset = adj_matrix[np.ix_(current_shuffle_idx, current_shuffle_idx)]
                    
                    # Add the subsetted adjacency matrix to the items list
                    items.append(torch.as_tensor(adj_subset, dtype=torch.float32))
            else:
    ...
    def _extract_data(self, data_configs: List[DATA_CONFIG]) -> Tuple[
            List[pd.Index], Tuple[
                List[AnyArray], List[AnyArray], List[AnyArray], List[AnyArray],
                List[AnyArray], List[AnyArray] # 1 less on purpose, special handling in getitem
            ]
    ]:
        if self.mode == "eval":
    ...
    def _extract_data_train(self, data_configs: List[DATA_CONFIG]) -> Tuple[
        ...
        xtmp = [
            self._extract_xtmp(adata, data_config)
            for adata, data_config in zip(self.adatas, data_configs)
        ]
        xlbl = [
        ...
        xadj = [
            self._extract_xadj(adata, data_config)
            for adata, data_config in zip(self.adatas, data_configs)
        ]
        return xuid, (x, xrep, xbch, xtmp, xlbl, xdwt, xadj)
    ...
    def _extract_xadj(self, adata: AnnData, data_config: DATA_CONFIG) -> AnyArray:
        default_dtype = get_default_numpy_dtype()
        adj_key = data_config["adjacency_key"]
        if adj_key not in adata.obsm:
            raise ValueError(
                f"Configured spatial adjacency matrix '{adj_key}' "
                f"cannot be found in input data!"
            )
        xadj = np.asarray(adata.obsm[adj_key]).astype(default_dtype)
        return xadj

    def _extract_xtmp(self, adata: AnnData, data_config: DATA_CONFIG) -> AnyArray:
        use_timepoint = data_config["use_timepoint"]
        timepoints = data_config["timepoints"]
        if use_timepoint:
            if use_timepoint not in adata.obs:
                raise ValueError(
                    f"Configured data timepoint '{use_timepoint}' "
                    f"cannot be found in input data!"
                )
            return timepoints.get_indexer(adata.obs[use_timepoint])
        return np.zeros(adata.shape[0], dtype=int)
...
class DataLoader(torch.utils.data.DataLoader):
    ...
    @staticmethod
    def _collate(batch):
        data, indices = zip(*batch)  # Split into data and observation indices
        return tuple(map(lambda x: torch.cat(x, dim=0), zip(*data))), np.concatenate(indices)
