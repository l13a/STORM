...
STORM, like its underlying GLUE framework, is designed to be modular and can be extended in many ways.
...
A STORM model is primarily composed of four main components (all PyTorch `Modules <https://pytorch.org/docs/stable/generated/torch.nn.Module.html>`__):

- `Data encoders <api/storm.models.sc.DataEncoder.rst>`__ (one for each domain)
...
- `Data decoders <api/storm.models.sc.DataDecoder.rst>`__ (one for each domain)
...
- A `graph encoder <api/storm.models.sc.GraphEncoder.rst>`__
...
- A `graph decoder <api/storm.models.sc.GraphDecoder.rst>`__
...
Current implementations for these components are all located in `storm.models.sc <api/storm.models.sc.rst>`__. New extensions can be added to this module as well.
...
Data encoders should inherit from the `DataEncoder <api/storm.models.sc.DataEncoder.rst>`__ class.
The main part of the encoder is an MLP (Multi-Layer Perceptron) already implemented in `DataEncoder <api/storm.models.sc.DataEncoder.rst>`__. It leaves two customizable abstract methods:
...
Data decoders should inherit from the `DataDecoder <api/storm.models.sc.DataDecoder.rst>`__ class. It defines the interface of the constructor as well as the abstract ``forward`` method.
...
Non-standard distributions can also be defined in `storm.models.prob <api/storm.models.prob.rst>`__.

Register custom encoder and decoder
-----------------------------------

Finally, use the `storm.models.storm.register_prob_model <api/storm.models.storm.register_prob_model.rst>`__ function to register the the custom encoder and decoder under a new "prob_model", so they can be activated with a matching ``prob_model`` setting in `configure_dataset <api/storm.models.configure_dataset.rst>`__.
